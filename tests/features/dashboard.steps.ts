...
When("I try to access {string}", async function (this: CustomWorld, path: string) {
...
Then("the Usuarios link should be active", async function (this: CustomWorld) {
  const link = this.page.locator("a[href='/core/usuarios']");
  await expect(link).toBeVisible();
  await expect(link).toHaveAttribute("class", /bg-accent/);
});

Then("I should see the Orgaos link", async function (this: CustomWorld) {
  await expect(this.page.locator("a[href='/core/orgaos']")).toBeVisible();
});

Then("I should see the Roles link", async function (this: CustomWorld) {
...
Then("my session should be cleared", async function (this: CustomWorld) {
